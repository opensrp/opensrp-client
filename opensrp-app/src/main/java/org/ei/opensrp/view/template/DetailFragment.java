...
public abstract class DetailFragment extends Fragment {
    ...
    public void displayPicture() {
        if (profilePicContainerId() != null) {
            ...
            if (allowImageCapture()) {
                ...
                ProfileImage photo = ((ImageRepository) Context.getInstance().imageRepository()).findByEntityId(client.entityId(), "dp");

                if (photo != null) {
                    setProfiePicFromPath(getActivity(), mImageView, photo.getFilepath(), R.drawable.ic_pencil);
                } else {
                    setProfiePic(getActivity(), mImageView, defaultProfilePicResId(), R.drawable.ic_pencil);
                }
            } else {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_TAKE_PHOTO && resultCode == getActivity().RESULT_OK) {
            ...
            setProfiePicFromPath(getActivity(), mImageView, currentPhoto.getAbsolutePath(), R.drawable.ic_pencil);
        }
    }
}

...
public class FieldMonitorRegisterFragment extends SecuredNativeSmartRegisterFragment {
    ...
    private List<String> customColumnsMonthly = new ArrayList<String>(){{
        add("(select count(id) c from pkwoman where SUBSTR(tt1,1,7) = SUBSTR(t.date,1,7)) tt1");
        add("(select count(id) c from pkwoman where SUBSTR(tt2,1,7) = SUBSTR(t.date,1,7)) tt2");
        add("(select count(id) c from pkwoman where SUBSTR(tt3,1,7) = SUBSTR(t.date,1,7)) tt3");
        add("(select count(id) c from pkwoman where SUBSTR(tt4,1,7) = SUBSTR(t.date,1,7)) tt4");
        add("(select count(id) c from pkwoman where SUBSTR(tt5,1,7) = SUBSTR(t.date,1,7)) tt5");
        add("(select count(id) c from pkchild where SUBSTR(bcg,1,7) = SUBSTR(t.date,1,7)) bcg");
        add("(select count(id) c from pkchild where SUBSTR(opv0,1,7) = SUBSTR(t.date,1,7)) opv0");
        add("(select count(id) c from pkchild where SUBSTR(opv1,1,7) = SUBSTR(t.date,1,7)) opv1");
        add("(select count(id) c from pkchild where SUBSTR(opv2,1,7) = SUBSTR(t.date,1,7)) opv2");
        add("(select count(id) c from pkchild where SUBSTR(opv3,1,7) = SUBSTR(t.date,1,7)) opv3");
        add("(select count(id) c from pkchild where SUBSTR(ipv,1,7) = SUBSTR(t.date,1,7)) ipv");
        add("(select count(id) c from pkchild where SUBSTR(pcv1,1,7) = SUBSTR(t.date,1,7)) pcv1");
        add("(select count(id) c from pkchild where SUBSTR(pcv2,1,7) = SUBSTR(t.date,1,7)) pcv2");
        add("(select count(id) c from pkchild where SUBSTR(pcv3,1,7) = SUBSTR(t.date,1,7)) pcv3");
        add("(select count(id) c from pkchild where SUBSTR(measles1,1,7) = SUBSTR(t.date,1,7)) measles1");
        add("(select count(id) c from pkchild where SUBSTR(measles2,1,7) = SUBSTR(t.date,1,7)) measles2");
        add("(select count(id) c from pkchild where SUBSTR(penta1,1,7) = SUBSTR(t.date,1,7)) penta1");
        add("(select count(id) c from pkchild where SUBSTR(penta2,1,7) = SUBSTR(t.date,1,7)) penta2");
        add("(select count(id) c from pkchild where SUBSTR(penta3,1,7) = SUBSTR(t.date,1,7)) penta3");
    }};
